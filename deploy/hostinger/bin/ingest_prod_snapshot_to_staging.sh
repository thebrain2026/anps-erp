...
docker cp "$DEST/anps_erp_${STAMP}.db" anps-erp-web:/data/anps_erp_source.db
docker compose --env-file "$BOUNDARY/secrets/secrets.env" -p anps_erp \
  -f "$BOUNDARY/staging/compose.yaml" exec -T --user root web chown anps:anps /data/anps_erp_source.db

docker compose --env-file "$BOUNDARY/secrets/secrets.env" -p anps_erp \
  -f "$BOUNDARY/staging/compose.yaml" exec -T web \
  python -u /app/migrate_sqlite_to_postgres.py /data/anps_erp_source.db

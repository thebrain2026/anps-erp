...
def initialize_outbox(conn):
    # Postgres cannot run the SQLite RAISE() immutability triggers. Use a
    # trigger-free schema there so capture_state_changes never blocks ERP saves.
    postgres_schema = """
CREATE TABLE IF NOT EXISTS bsfv_source_versions (
    aggregate_type TEXT NOT NULL,
    aggregate_id TEXT NOT NULL,
...
"""
    try:
        from anps_db import using_postgres
        if using_postgres():
